...
int main(int argc, char *argv[])
{
	if (argc != 8)
	{
		printf("Usage:%s model_path function_name dev_id dev_channel image_path is_rgb output_filename\n", argv[0]);
		printf("\nex.  ./cnrt_simple_demo ../resnet18_intx.cambricon subnet0 0 0 ../images/image.jpg\n\n");
		return -1;
	}
	...
	const char *output_filename = argv[7]; //输出保存
	...
	for (int i = 0; i < batch_size; i++)
	{
		...
		cv::resize(input_image, input_image_resized, cv::Size(input_width, input_height), CV_INTER_CUBIC);
		if (is_rgb == 1)
		{
